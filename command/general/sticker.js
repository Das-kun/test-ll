	...
	async exec({ msg, sock }) {
		...
		try {
			if ((isMedia && !msg.message.videoMessage) || isQImg) {
				...
				await msg.reply(`Please send/reply the image/video/document you want to convert to a sticker.\nMake sure the size does not exceed 2MB and max duration is 10second(s).`);
			}
